...
use auxide_dsp::nodes::filters::{LadderFilter, SvfFilter};
use auxide_dsp::nodes::fx::Delay;
use auxide_dsp::nodes::lfo::Lfo;
use auxide_dsp::nodes::oscillators::{
    BrownNoise, Constant, PinkNoise, PulseOsc, SawOsc, SquareOsc, SuperSaw, TriangleOsc,
    WavetableOsc, WhiteNoise,
};
...
fn test_all_nodes_rt_safe() {
    ...
    let square = graph_square.add_external_node(SquareOsc {
        freq: 440.0,
        pulse_width: 0.5,
    });
    ...
    let pulse = graph_pulse.add_external_node(PulseOsc {
        freq: 440.0,
        pulse_width: 0.5,
    });
    ...
    let wavetable = graph_wavetable.add_external_node(WavetableOsc {
        freq: 440.0,
        table: std::sync::Arc::new(vec![0.0, 1.0, 0.0, -1.0]),
    });
    ...
    let supersaw = graph_supersaw.add_external_node(SuperSaw {
        freq: 440.0,
        detune: 0.1,
        voices: 3,
    });
    ...
    runtime_whitenoise
        .process_block(&mut out_whitenoise)
        .unwrap();
    ...
    runtime_brownnoise
        .process_block(&mut out_brownnoise)
        .unwrap();
    ...
    runtime_compressor
        .process_block(&mut out_compressor)
        .unwrap();
    ...
    runtime_waveshaper
        .process_block(&mut out_waveshaper)
        .unwrap();
    ...
    runtime_pitchshifter
        .process_block(&mut out_pitchshifter)
        .unwrap();
    ...
    let ringmod = graph_ringmod.add_external_node(RingMod { mix: 1.0 });
